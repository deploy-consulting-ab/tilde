...
/**
 * Build vacation balance from manually maintained user profile fields.
 * HR enters ingoing pool values at semester start (1 Apr). Used days come from Flex absences.
 * @param {Object} params
 * @param {number} [params.vacationEarnedDays]
 * @param {number} [params.vacationSavedDays]
 * @param {number} [params.vacationAdvanceDays]
 * @param {number} [params.usedDays]
 * @returns {Object}
 */
export const buildVacationBalanceFromProfile = ({
    vacationEarnedDays = 0,
    vacationSavedDays = 0,
    vacationAdvanceDays = 0,
    usedDays = 0,
}) => {
    const earnedDays = vacationEarnedDays ?? 0;
    const savedDays = vacationSavedDays ?? 0;
    const advanceDays = vacationAdvanceDays ?? 0;
    const used = Math.max(0, usedDays ?? 0);
    const totalDays = earnedDays + savedDays + advanceDays;
    const availableDays = totalDays - used;
...
/**
 * Extract holiday fields from session user for getHolidays().
 * @param {Object} user
 * @returns {Object}
 */
export const getHolidayEmployeeInfo = (user) => ({
...
/**
 * Format vacation day counts for display (whole numbers or one decimal).
...
export const transformHolidaysData = (rawData) => {
    ...
    const balance = rawData.balance ?? null;

    return {
        availableDays: balance?.availableDays ?? rawData.availableHolidays ?? 0,
        totalDays: balance?.totalDays ?? rawData.totalHolidays ?? 0,
        usedDays: balance?.usedDays ?? rawData.currentFiscalUsedHolidays ?? 0,
        earnedDays: balance?.earnedDays ?? null,
        savedDays: balance?.savedDays ?? rawData.carriedOverHolidays ?? 0,
        advanceDays: balance?.advanceDays ?? null,
        isBalanceManual: balance?.isManual ?? false,
 ...
 */
/**
 * Compare two arbitrary fiscal-year/quarter periods.
 * Returns base row (no delta) and target row with percent change vs base.
 * @param {Array} records
 * @param {{ fiscalYear: number, quarter: number }} basePeriod
 * @param {{ fiscalYear: number, quarter: number }} comparePeriod
 * @returns {Array}
 */
export function buildCustomPeriodComparison(records, basePeriod, comparePeriod) {
    const base = getFinancialRecord(records, basePeriod.fiscalYear, basePeriod.quarter);
    const compare = getFinancialRecord(records, comparePeriod.fiscalYear, comparePeriod.quarter);

    const rows = [];

    if (base) {
        rows.push({ ...base, _isComparisonBase: true });
    }

    if (compare) {
        const yoy = base
            ? {
                  revenue: calculatePercentChange(compare.revenue, base.revenue),
                  cost: calculatePercentChange(compare.cost, base.cost),
                  profit: calculatePercentChange(compare.profit, base.profit),
                  taxes: calculatePercentChange(compare.taxes, base.taxes),
              }
            : null;

        rows.push({ ...compare, _yoy: yoy, _isComparisonTarget: true });
    }
...
/**
 * Attach YoY percent changes independently for each selected quarter.
 * Rows are sorted by fiscal year descending, then quarter ascending.
 * @param {Array} records
 * @param {number[]} quarters
 * @returns {Array}
 */
export function attachYearOverYearChangesForQuarters(records, quarters) {
    return quarters
        .flatMap((quarter) => attachYearOverYearChanges(records, [quarter]))
        .toSorted((a, b) => {
